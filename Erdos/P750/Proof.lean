...
proper) follows immediately and is `Erdos750.erdos_750_independence`.

Trust boundary beyond Mathlib core (`propext`, `Quot.sound`, `Classical.choice`):
  Erdos750.stiebitz_lower_bound — Stiebitz's theorem on χ(generalized Mycielski).
-/
...
lemma MycAdj_irrefl (s : ℕ) {V : Type u} (G : SimpleGraph V) :
    Irreflexive (MycAdj s G) := by
  intro a h
  match a, h with
  | Sum.inl (i, v), h =>
      simp only [MycAdj] at h
      rcases h with ⟨_, _, h⟩ | ⟨hii, _⟩ | ⟨hii, _⟩
      · exact G.loopless v ‹_›
      · omega
      · omega
  | Sum.inr (), h =>
      simp only [MycAdj] at h
...
/-! ## §2 (cont.) Recursive class `Mᵣ` and Stiebitz's axiom -/
...
/--
**Stiebitz's theorem** (axiom). Every graph in the recursively built class `Mᵣ` has
...
def genMycMinusZero {V : Type u} (s : ℕ) (G : SimpleGraph V) :
  ...
  loopless := fun a ⟨h1, _⟩ => (genMyc s G).loopless a h1
  ...
  | succ r hr IH =>
      ...
      · -- The OCT bound.
        ...
        · -- Case 2: g X.card = 0.
          ...
          have hwHlen : wH.length = w.length := by simp [hwH_def]
          have hwHsupp : wH.support = w.support.map f := by simp [hwH_def]
...
on the Stiebitz axiom (unlike `finite_oct_profile` itself, which is axiom-free). -/
...
Trust boundary check: every public theorem depends only on Mathlib core
(`propext`, `Classical.choice`, `Quot.sound`) plus the single axiom
`stiebitz_lower_bound`. Uncomment the lines below to reproduce at build time.
-/

-- #print axioms genMyc_chromaticNumber_le_succ          -- (Mathlib core only)
-- #print axioms oct_mono_edges                          -- (Mathlib core only)
-- #print axioms oct_genMyc_le                           -- (Mathlib core only)
-- #print axioms genMyc_oddClosedWalk_through_apex_long  -- (Mathlib core only)
-- #print axioms genMyc_oddCycle_through_apex_long       -- (Mathlib core only)
-- #print axioms finite_nonbipartite_induce_has_short_odd_cycle -- (Mathlib core only)
-- #print axioms finite_oct_profile                      -- (Mathlib core only — does NOT use Stiebitz)
-- #print axioms finite_oct_profile_with_chromatic       -- (Mathlib core + stiebitz_lower_bound)
-- #print axioms infinite_chromatic_local_oct            -- (Mathlib core + stiebitz_lower_bound)
-- #print axioms erdos_750_independence                  -- (Mathlib core + stiebitz_lower_bound)
-- #print axioms erdos_750_independence_FC_form          -- (Mathlib core + stiebitz_lower_bound)
-- #print axioms erdos_750_FC                            -- (Mathlib core + stiebitz_lower_bound)
